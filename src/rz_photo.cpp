...
#define CATCH_EXIV2 catch (Exiv2::Error& e) { qWarning() << "Exiv2 Error:" << e.what(); return false; }
...
bool Photo::isValidImageType() {
    return Photo::allSupportedTypes.contains(imgStruct.fileSuffix, Qt::CaseInsensitive);
}

bool Photo::hasMetadataSupport() {
    return Photo::metaSupportedTypes.contains(imgStruct.fileSuffix, Qt::CaseInsensitive);
}
...
QString Photo::determineTimestamp() {
    ...
    if (hasMetadataSupport()) {
        try {
            auto image = Exiv2::ImageFactory::open(imgStruct.fileAbsolutePath.toStdString());
            image->readMetadata();
            ...
            if (!exifData.empty()) {
                ...
                } else if (exifData.findKey(Exiv2::ExifKey("Exif.Image.DateTime")) != exifData.end()) {
    ...
    if (!finalDate.isValid()) {
        ...
        if (!finalDate.isValid()) finalDate = fi.lastModified();
    }
...
const bool Photo::convertImage(const int &targetSize, const int &quality) {
    if (!isValidImageType()) return false;
    if (!createWebpPath()) return false;

    // NEU: Namens-Logik (Timestamp Rename)
    ...
    QString sizeSuffix = (targetSize > 0) ? "_" + QString::number(targetSize) : "";
    QString targetPath = imgStruct.fileAbsolutePath.left(imgStruct.fileAbsolutePath.lastIndexOf('/')) 
                         + "/" + imgStruct.newFolder + "/" 
                         + targetBasename + sizeSuffix + "." + imgStruct.newSuffix;

    QFile targetFile(targetPath);
    if (targetFile.exists() && !overwriteExitingWebp_bool) return true; 
    ...
    if (targetSize > 0) {
        ...
        if (originalSize.width() > targetSize || originalSize.height() > targetSize) {
             reader.setScaledSize(originalSize.scaled(targetSize, targetSize, Qt::KeepAspectRatio));
        }
    }
    ...
    if (targetSize > 0 && oversizeSmallerPicture_bool) {
        if (img.width() < targetSize && img.height() < targetSize) {
            img = img.scaled(targetSize, targetSize, Qt::KeepAspectRatio, Qt::SmoothTransformation);
        }
    }
...
const std::tuple<bool, QString> Photo::renameImageToTimestamp() {
    ...
    // Nutzt die Helper Methode (Code Reuse)
    ...
    while (QFile::exists(newPath)) {
        if (newPath == imgStruct.fileAbsolutePath) return {true, newPath};
        newPath = dirPath + "/" + newName + "_" + QString::number(counter++) + "." + imgStruct.fileSuffix;
    }
...
bool Photo::writeXmp(const QString &key, const QString &value) {
    if (!hasMetadataSupport()) return false;
    try {
        auto image = Exiv2::ImageFactory::open(imgStruct.fileAbsolutePath.toStdString());
        image->readMetadata();
...
bool Photo::writeAllMetadata(const RzMetadata::DefaultMetaStruct &metaData) {
    ...
    try {
        ...
        while (itExif.hasNext()) {
            itExif.next();
            if (!itExif.value().isEmpty()) exifData[itExif.key().toStdString()] = itExif.value().toStdString();
        }

        QHashIterator<QString, QString> itIptc(metaData.iptcDefault);
        while (itIptc.hasNext()) {
            itIptc.next();
            if (!itIptc.value().isEmpty()) iptcData[itIptc.key().toStdString()] = itIptc.value().toStdString();
        }

        QHashIterator<QString, QString> itXmp(metaData.xmpDefault);
        while (itXmp.hasNext()) {
            itXmp.next();
            if (!itXmp.value().isEmpty()) xmpData[itXmp.key().toStdString()] = itXmp.value().toStdString();
        }
...
QString Photo::getRawTagValue(const QString &key) {
    ...
    try {
        ...
        if (key.startsWith("Exif.")) {
            if (image->exifData().findKey(Exiv2::ExifKey(key.toStdString())) != image->exifData().end())
                return QString::fromStdString(image->exifData()[key.toStdString()].toString());
        } else if (key.startsWith("Iptc.")) {
            if (image->iptcData().findKey(Exiv2::IptcKey(key.toStdString())) != image->iptcData().end())
                return QString::fromStdString(image->iptcData()[key.toStdString()].toString());
        } else if (key.startsWith("Xmp.")) {
            if (image->xmpData().findKey(Exiv2::XmpKey(key.toStdString())) != image->xmpData().end())
                return QString::fromStdString(image->xmpData()[key.toStdString()].toString());
        }
    } catch (...) {}
    return "";
}
...
QString Photo::getXmpCopyrightOwner() {
    ...
    try {
        ...
        if (xmpData.findKey(Exiv2::XmpKey("Xmp.plus.CopyrightOwner")) != xmpData.end()) {
             return QString::fromStdString(xmpData["Xmp.plus.CopyrightOwner"].toString());
        }
...
bool Photo::writeToAllCopyrightOwner(const QString &owner) {
    ...
    if (RzMetadata::xmp_to_exif.contains("Xmp.dc.creator")) {
        ok &= writeExif(RzMetadata::xmp_to_exif["Xmp.dc.creator"], owner);
    }
    return ok;
}
...
static double convertToDecimal(const Exiv2::ExifData &exifData, const std::string &keyVal, const std::string &keyRef) {
    try {
        auto pos = exifData.findKey(Exiv2::ExifKey(keyVal));
        if (pos == exifData.end()) return 0.0;
        
        auto v = pos->getValue(); 
        if (v->count() != 3) return 0.0;
...
QPointF Photo::getGpsLatLon() {
    if (!hasMetadataSupport()) return QPointF(0.0, 0.0);
    try {
        auto image = Exiv2::ImageFactory::open(imgStruct.fileAbsolutePath.toStdString());
        image->readMetadata();
        Exiv2::ExifData &exifData = image->exifData();
        
        if (exifData.empty()) return QPointF(0.0, 0.0);

        double lat = convertToDecimal(exifData, "Exif.GPSInfo.GPSLatitude", "Exif.GPSInfo.GPSLatitudeRef");
        double lon = convertToDecimal(exifData, "Exif.GPSInfo.GPSLongitude", "Exif.GPSInfo.GPSLongitudeRef");

        return QPointF(lat, lon);
    } catch (...) { return QPointF(0.0, 0.0); }
}

Photo::ExifGpsStruct Photo::getGpsData() const {
    ExifGpsStruct gps;
    if (!const_cast<Photo*>(this)->hasMetadataSupport()) return gps;
    try {
        auto image = Exiv2::ImageFactory::open(imgStruct.fileAbsolutePath.toStdString());
        image->readMetadata();
        Exiv2::ExifData &exifData = image->exifData();
        
        if (exifData.empty()) return gps;
...
QString Photo::getGpsString() {
    ...
    try {
        ...
        if (exifData.findKey(keyLat) == exifData.end()) return "";

        return QString::fromStdString(exifData["Exif.GPSInfo.GPSLatitude"].toString() + " " + 
...
QList<QString> Photo::srcPics(const QString &srcPath) {
    ...
    for (const auto& fi : dir.entryInfoList()) {
        if (isImageExtensionValid(fi)) list << fi.absoluteFilePath();
    }
    return list;
}
...
QList<QString> Photo::srcPicsRecursive(const QString &srcPath) {
    ...
    while (it.hasNext()) {
        it.next();
        if (isImageExtensionValid(it.fileInfo())) list << it.filePath();
    }
    return list;
}
